...
class ContentCompiler:
    ...
    def discover_categories(self):
        # Walk directory hierarchy under CONTENT_DIR
        for root, dirs, files in os.walk(self.content_dir):
            ...
            for depth, part in enumerate(parts, 1):
                ...
                cat_id = self.slugify("-".join(path_accum))
                cat_path = " → ".join(path_accum)
        ...
        for root, dirs, files in os.walk(self.content_dir):
            ...
            for file in files:
                ...
                try:
                    ...
                    self.error(rel_file_path, f"Failed to read file: {e}", 1, "Check file permissions and encoding")
                    continue

                if not raw_content.strip():
                    self.error(rel_file_path, "Empty article content", 1, "Add article content and headings")
                    continue

                fm, body = self.parse_front_matter(raw_content)
                
                # Derive Article ID
                article_id = fm.get("id") or file_path.stem
                article_id = self.slugify(article_id)

                if article_id in self.articles:
                    self.error(rel_file_path, f"Duplicate article ID: '{article_id}'", 1, "Change the ID in front matter or rename the file")
                    continue

                # Title
                title = self.extract_title(file_path, body, fm.get("title"))
                if not title:
                    self.error(rel_file_path, "Missing title or H1 heading", 1, "Add a '# Title' heading")
                    continue
